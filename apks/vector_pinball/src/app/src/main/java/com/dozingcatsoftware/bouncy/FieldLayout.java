...
public class FieldLayout {
    ...
    static Application _appContext;
    Random RAND = new Random();

    private FieldLayout() {}

    public static void setContext(Application appContext) {
    ...
    public static int numberOfLevels() {
        ...
        try {
            List<String> tableFiles = Arrays.asList(_appContext.getAssets().list("tables"));
            int count = 0;
    ...
    static Map<String, Object> readFieldLayout(int level) {
        try {
            ...
            InputStream fin = _appContext.getAssets().open(assetPath);
    ...
    public int getNumberOfBalls() {
        if (RLTask.get().isEnabled()) {
            return RLTask.get().get("numballs", this.numberOfBalls);
        }
